...
> **STATUS (2026-07-29): V1 Phase-2 run ARCHIVE — not a live runbook.** The run was manually
> soft-stopped at E5 (0-based) of 15; rows E6+ will never be filled. Kept because it is the only
> per-epoch record of V1-P2 metrics (cited by the v2 evidence base) and because
> `tools/run_validation_for_epoch.py` still emits rows in this file's format. The canary
> methodology below is the template for V2 monitoring. Operational guidance is superseded by
> [todos/v2-plan.md](todos/v2-plan.md).

## Run context

- **Phase:** Phase 2 (448×448 fine-tune) of the two-stage plan in `todos/progressive-training-plan.md` (retired 2026-07-29, full text in git history; superseded by [todos/v2-plan.md](todos/v2-plan.md)). Optimizer reset, bicubic pos_embed interpolation 20×20 → 28×28, reduced regularization, `gamma_neg=7.0`, `clip=0.2`, peak LR 1.4e-5, `num_epochs=15`, `warmup_epochs=2`. Phase 1 best (E32, step 209135) checkpoint is the starting point.
